...
public:
    ...
    // CV_min(CV_min_in), CV_max(CV_max_in), bin_number(bin_number_in), grid_point_number(bin_number_in + 1) weight(weight_in), well_tempered_parameter(well_tempered_parameter_in), threshold_weight(threshold_weight_in)
    // {
    //     assert(CV_min_in < CV_max_in);
    //     // Resize bias_grid and set all entries to 0
    //     bias_grid.assign(grid_point_number, 0.0);
    ...
    //     std::cout << "  grid_point_number: " << grid_point_number << "\n";
    ...
    void GeneratePotentialFrom(funcT&& generator_function) noexcept
    {
        ...
        {
            bias_grid[bin_index] = generator_function(CVFromBinIndex(bin_index));
        }
    }
    ...
    void UpdatePotential(const double CV) noexcept
    {
        ...
        {
            double CV_current_bin {CVFromBinIndex(bin)};
            if constexpr(metapotential_well_tempered)
            {
                bias_grid[bin] += weight * std::exp(-0.5 * bin_width_inverse * bin_width_inverse * std::pow(CV - CV_current_bin, 2) - bias_grid[bin] / well_tempered_parameter);
            }
            else
            {
                bias_grid[bin] += weight * std::exp(-0.5 * bin_width_inverse * bin_width_inverse * std::pow(CV - CV_current_bin, 2));
            }
        }
    }
    ...
    double ReturnPotential(const double CV) const noexcept
    {
        ...
        else
        {
            ...
            if (bin_index < 0)
            {
                return bias_grid.front() + threshold_weight * std::pow((CV - CV_min), 2);
            }
            else
            {
                return bias_grid.back()  + threshold_weight * std::pow((CV - CV_max), 2);
            }
        }
    }
    ...
    {
        // At the time of writing this I'm too lazy to write this without a copy (bias_grid_reverse), and this way it at least looks cleaner than manually looping through bias_grid
        std::vector<double> bias_grid_reverse = bias_grid;
        std::reverse_copy(bias_grid.cbegin(), bias_grid.cend(), bias_grid_reverse.begin());
        std::transform(bias_grid.begin(), bias_grid.end(), bias_grid_reverse.cbegin(), bias_grid.begin(), [](auto element, auto element_reverse){return 0.5 * (element + element_reverse);});
        std::cout << "Metapotential symmetrized (average value)!" << std::endl;
    }
    ...
    void SymmetrizePotentialMaximum() noexcept
    {
        ...
        std::transform(bias_grid.begin(), bias_grid.end(), bias_grid_reverse.cbegin(), bias_grid.begin(), [](auto element, auto element_reverse){return std::max(element, element_reverse);});
        std::cout << "Metapotential symmetrized (max value)!" << std::endl;
    }
    ...
    void SubtractPenaltyWeight(const double CV_lower, const double CV_upper) noexcept
    {
        ...
        {
            double CV {CVFromBinIndex(ind)};
            bias_grid[ind] -= threshold_weight * std::pow((CV - CV_lower), 2);
        }
        for (int ind = upper_index; ind < grid_point_number; ++ind)
        {
            double CV {CVFromBinIndex(ind)};
            bias_grid[ind] -= threshold_weight * std::pow((CV - CV_upper), 2);
        }
    }
    ...
    double ReturnDerivative(const double CV) const noexcept
    {
        ...
        // Values equal to grid_point_number - 1 are not allowed, since we interpolate between the bin_index and bin_index + 1
        // Effectively means that our potential is limited to the interval [CV_min, CV_max)
        // TODO: Shouldn't we calculate the difference between ReturnPotential(bin_index + 1) and bin_index instead of bias_grid?
        ...
        //     // return 0.5 * (bias_grid[bin_index + 1] - bias_grid[bin_index]);
        //     return bin_width_inverse * (bias_grid[bin_index + 1] - bias_grid[bin_index]);
        // }
        // TODO:
        // else
        // {
    ...
    void SaveParameters(const std::string& filename, const bool overwrite = false)
    {
        if (overwrite)
        {
            bias_ostream.open(filename, std::fstream::out | std::fstream::trunc);
        }
        else
        {
            bias_ostream.open(filename, std::fstream::out | std::fstream::app);
        }
        bias_ostream << program_version << "\n";
        bias_ostream << "Metadynamics Potential" << "\n";
        bias_ostream << "CV_min: " << CV_min << "\n";
        bias_ostream << "CV_max: " << CV_max << "\n";
        bias_ostream << "bin_number: " << bin_number << "\n";
        bias_ostream << "weight: " << weight << "\n";
        // TODO: Include well_tempered_parameter?
        bias_ostream << "well_tempered_parameter: " << well_tempered_parameter << "\n";
        bias_ostream << "threshold_weight: " << threshold_weight << "\n";
        bias_ostream << "END_METADYN_PARAMS" << "\n";
        bias_ostream.close();
        bias_ostream.clear();
    }
    ...
    void SavePotential(const std::string& filename, const bool overwrite = false)
    {
        ...
        std::copy(bias_grid.cbegin(), std::prev(bias_grid.cend()), std::ostream_iterator<double>(bias_ostream, ","));
        bias_ostream << bias_grid.back() << "\n";
        bias_ostream << "exceeded_count: " << exceeded_count << "\n";
        bias_ostream.close();
        bias_ostream.clear();
    }
    ...
    bool LoadPotential(const std::string& filename)
    {
        ...
        std::size_t linecount = std::count(std::istreambuf_iterator<char>(bias_istream), std::istreambuf_iterator<char>(), '\n') + 1;
        // Return to beginning of file
        bias_istream.clear();
        bias_istream.seekg(0, bias_istream.beg);
        // Start reading parameters
        // std::getline(bias_istream, current_line);
        ...
        if (true)
        {
            ...
            while(std::getline(bias_istream, current_line))
            {
            ...
            {
                bias_istream.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            }
            // Load histogram into bias_grid
            bias_grid.clear();
            while (std::getline(bias_istream, current_line, ','))
            {
                bias_grid.push_back(std::stod(current_line));
            }
            // Get exceeded count
            ...
            std::cout << "  bias_grid.size():        " << bias_grid.size() << "\n";
            std::cout << "  bin_width:               " << bin_width << "\n";
            std::cout << "  bin_width_inverse:       " << bin_width_inverse << "\n";
            std::cout << "  weight:                  " << weight << "\n";
            std::cout << "  well_tempered_parameter: " << well_tempered_parameter << "\n";
